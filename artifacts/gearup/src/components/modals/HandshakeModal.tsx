...
export default function HandshakeModal({ rental, onClose, userRole, initialStep }: HandshakeModalProps) {
  ...
  const markPaymentFailed = async (reason: string, orderId?: string, paymentId?: string, signature?: string) => {
    try {
      await updateDoc(doc(db, 'rentals', rental.id), {
  ...
  const completeVerifiedPayment = async (payment: { orderId: string; paymentId: string; signature: string }) => {
    await updateDoc(doc(db, 'rentals', rental.id), {
  ...
  const handleRazorpayPayment = async () => {
    ...
    try {
      ...
      const checkout = new window.Razorpay({
        ...
        handler: async (response) => {
          ...
          try {
            ...
            showToast('Payment verified. Rental is now active.', 'success');
            onClose();
          } catch (err: any) {
            ...
            await markPaymentFailed(
              err?.message || 'verification_failed',
              response.razorpay_order_id,
              response.razorpay_payment_id,
              response.razorpay_signature
            );
            showToast('Payment could not be verified. Rental is still pending.', 'error');
          } finally {
            setLoading(false);
          }
        },
        modal: {
          ondismiss: async () => {
            ...
            await markPaymentFailed('checkout_dismissed', order.orderId);
            setLoading(false);
      ...
      console.error('Razorpay order setup failed:', {
        code: err?.code,
        message: err?.message,
        error: err,
      });
             ...
             {step === 'payment_scan' && (
               <motion.div key="scan" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-6">
                  <div className="text-center space-y-2">
                    ...
                    <p className="text-[13px] text-white/50">Create a Razorpay order and verify payment before the rental activates.</p>
                  </div>
